...
describe('endpoints de lectura', () => {
  it('fetchSyncStatus consulta /api/biblia/sync/status', async () => {
    ...
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/sync/status');
  });
  ...
  it('fetchPreparaciones arma la query con todos los filtros', async () => {
    ...
    expect(requestMock).toHaveBeenCalledWith(
      '/api/distrigestion/biblia/preparaciones?fecha_desde=2026-07-01&fecha_hasta=2026-07-02&estado=Completada&tipo=Pedidos+individuales&page=2&pageSize=50',
    );
  });

  it('fetchPreparaciones omite los filtros ausentes', async () => {
    await fetchPreparaciones({ fecha_desde: '2026-07-01' });
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/preparaciones?fecha_desde=2026-07-01');
  });

  it('fetchChoferes sin params pega sin query string', async () => {
    await fetchChoferes();
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/choferes');
  });
  ...
  it('fetchChoferes con params arma la query', async () => {
    ...
    expect(requestMock).toHaveBeenCalledWith(
      '/api/distrigestion/biblia/choferes?fecha_desde=2026-07-01&fecha_hasta=2026-07-02&biblia_fecha=2026-07-03',
    );
  });

  it('fetchCodigosDespacho consulta /api/biblia/repartos', async () => {
    await fetchCodigosDespacho();
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/repartos');
  });

  it('fetchAsignaciones consulta /api/biblia/asignaciones', async () => {
    await fetchAsignaciones();
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/asignaciones');
  });

  it('searchChoferes escapa la query', async () => {
    await searchChoferes('lópez & co');
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/choferes/search?q=l%C3%B3pez%20%26%20co');
  });

  it('searchCodigosDespacho escapa la query', async () => {
    await searchCodigosDespacho('PERI 5');
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/repartos/search?q=PERI%205');
  });

  it('fetchRepartosExcepcionales con y sin fecha', async () => {
    await fetchRepartosExcepcionales('2026-07-10');
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/repartos-excepcionales?fecha=2026-07-10');
    await fetchRepartosExcepcionales();
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/repartos-excepcionales');
  });

  it('fetchRangoBiblia pasa biblia_fecha', async () => {
    await fetchRangoBiblia('2026-07-10');
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/reportes/rango-biblia?biblia_fecha=2026-07-10');
  });

  it('fetchResumenBiblia pasa biblia_fecha', async () => {
    await fetchResumenBiblia('2026-07-10');
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/reportes/resumen-biblia?biblia_fecha=2026-07-10');
  });
  ...
  it('fetchRepartosPorDireccion duplica fecha_desde como fecha_hasta si falta', async () => {
    ...
    expect(requestMock).toHaveBeenCalledWith(
      '/api/distrigestion/biblia/reportes/repartos-por-direccion?fecha_desde=2026-07-10&fecha_hasta=2026-07-10',
    );
  });
  ...
  it('fetchRepartosPorDireccion incluye biblia_fecha cuando se pasa', async () => {
    ...
    expect(requestMock).toHaveBeenCalledWith(
      '/api/distrigestion/biblia/reportes/repartos-por-direccion?fecha_desde=2026-07-09&fecha_hasta=2026-07-10&biblia_fecha=2026-07-11',
    );
  });
  ...
  it('fetchMapaClientes duplica fecha_desde y agrega biblia_fecha opcional', async () => {
    ...
    expect(requestMock).toHaveBeenCalledWith(
      '/api/distrigestion/biblia/reportes/mapa-clientes?fecha_desde=2026-07-10&fecha_hasta=2026-07-10',
    );
    await fetchMapaClientes('2026-07-09', '2026-07-10', '2026-07-11');
    expect(requestMock).toHaveBeenCalledWith(
      '/api/distrigestion/biblia/reportes/mapa-clientes?fecha_desde=2026-07-09&fecha_hasta=2026-07-10&biblia_fecha=2026-07-11',
    );
  });

  it('fetchCambiosEstado pasa biblia_fecha', async () => {
    await fetchCambiosEstado('2026-07-10');
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/sigma/cambios-estado?biblia_fecha=2026-07-10');
  });

  it('fetchAllChoferes consulta /api/biblia/choferes/all', async () => {
    await fetchAllChoferes();
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/choferes/all');
  });
  ...
  it('fetchAllZonas consulta /api/biblia/zonas', async () => {
    await fetchAllZonas();
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/zonas');
  });
});
...
describe('endpoints de escritura', () => {
  it('asignarPreparacionAChofer hace PUT con el body completo', async () => {
    ...
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/asignaciones/7/chofer', {
      method: 'PUT',
      body: JSON.stringify({
        chofer_codigo: 'CH1',
  ...
  it('asignarPreparacionesBatch hace POST a /batch', async () => {
    ...
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/asignaciones/batch', {
      method: 'POST',
  ...
  it('eliminarRepartoExcepcional hace DELETE por id', async () => {
    ...
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/repartos-excepcionales/42', { method: 'DELETE' });
  });

  it('limpiarAsignacionesBiblia hace DELETE por fecha', async () => {
    await limpiarAsignacionesBiblia('2026-07-10');
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/asignaciones/biblia/2026-07-10', { method: 'DELETE' });
  });
  ...
  it('updateChofer hace PUT escapando el código', async () => {
    ...
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/choferes/CH%209', {
      method: 'PUT',
  ...
  it('updateCodigoDespacho hace PUT escapando el id', async () => {
    ...
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/repartos/PERI%205', {
      method: 'PUT',
  ...
  it('asignarChoferCodigoDespacho hace POST al sub-recurso choferes', async () => {
    ...
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/repartos/BIG/choferes', {
      method: 'POST',
  ...
  it('desasignarChoferCodigoDespacho hace DELETE al sub-recurso', async () => {
    ...
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/repartos/BIG/choferes/CH1', { method: 'DELETE' });
  });
  ...
  it('updateZona hace PUT por id', async () => {
    ...
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/zonas/3', {
      method: 'PUT',
  ...
  it('asignarCodigoDespachoAZona delega en updateCodigoDespacho con zona_id', async () => {
    ...
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/repartos/BIG', {
      method: 'PUT',
  ...
  it('modificarPedido hace PUT con el body armado', async () => {
    ...
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/pedidos/P-1%2F2/reparto', {
      method: 'PUT',
      body: JSON.stringify({
        repartoId: 9,
        nuevoCodigo: 'BIG',
  ...
  it('impactarEnSigma hace POST con biblia_fecha', async () => {
    ...
    expect(requestMock).toHaveBeenCalledWith('/api/distrigestion/biblia/sigma/impactar', {
      method: 'POST',
...
describe('suscriptores SSE', () => {
  ...
  it('subscribeSyncAll abre la URL del stream full (la sesión viaja en cookies, sin token en query)', () => {
    subscribeSyncAll(() => {});
    expect(lastES().url).toBe('/api/distrigestion/biblia/sync/all/stream');
  });

  it('subscribeSyncHoy abre la URL del stream hoy', () => {
    subscribeSyncHoy(() => {});
    expect(lastES().url).toBe('/api/distrigestion/biblia/sync/hoy/stream');
  });
  ...
  it('subscribeSyncEvents notifica solo ante sync-complete', () => {
    ...
    expect(lastES().url).toBe('/api/distrigestion/biblia/sync/events');
